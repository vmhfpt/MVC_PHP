...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1>Danh sách biến thể sản phẩm "<?=$item['product_name']?>"</h1>
            </div>
            <div class="col-sm-6">
                                      ...
                                      <?php foreach($dataItem as $key => $value){?>
                                        ...
                                        <tr id="<?=$value['id']?>">
                                                ...
                                                <td class="project-actions text-right">
                                                   ...
                                                   <a class="btn btn-success btn-sm" href="/admin/color-product/<?=$value['id']?>">
                    ...
                    <div class="row">
                        <div class="col-md-4"> 
                            <div class="form-group">
                                ...
                                <input value="<?=$item['product_id']?>" name="product_id" type="hidden" class="">
                                <select name="type_id" class="get-value custom-select">
                                       <option value="">-- Lựa chọn --</option>
                                        <?php foreach($listAttribute as $key => $value){?>
                                            <option  <?= isset($old_field['type_id']) && $old_field['type_id'] == $value['type_id'] ? 'selected' : ''?> value="<?=$value['type_id']?>"><?=$value['description']?></option>
                                        <?php }?>
                                </select>
                                <?= isset($errors['type_id']) ? '<span id="error-name" class="text-danger">'.$errors['type_id'].'</span>' : ''?>
                            </div>
                        </div>
                        <div class="col-md-4"> 
                            <div class="form-group">
                                <label>GIÁ TRỊ</label>
                                <select name="value_id" class="custom-select" id="result" >
                                <option value="">-- Lựa chọn --</option>
                                </select>
                                <?= isset($errors['value_id']) ? '<span id="error-name" class="text-danger">'.$errors['value_id'].'</span>' : ''?>
                            </div>
                        </div>
                        <div class="col-md-4">
...
<script>
 var idDelete = false;
    function deleteAjax(dataPayload) {
        $.ajax({
                method: "POST",
                url: "/admin/attribute-product/delete",
                data: {
                    arr: dataPayload,
                }
            })
