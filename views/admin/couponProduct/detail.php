...
<section class="content-header">
    <div class="container-fluid">
        <div class="row mb-2">
            <div class="col-sm-6">
                <h1>Danh sách mã giảm giá sản phẩm "<?=$item['name']?>"</h1>
            </div>
            <div class="col-sm-6">
                                    ...
                                    <tbody>
                                      <?php foreach($dataItem as $key => $value){?>
                                        ...
                                        <tr id="<?=$value['id']?>">
                                                ...
                                                <td class="ten_loai"><?=$value['coupon_name']?></td>
                                                <td ><?=$value['code']?></td>
                    ...
                    <div class="row">
                        <div class="col-md-4"> 
                            <div class="form-group">
                                ...
                                <select name="coupon_id" class="get-value custom-select">
                                       <option value="">-- Lựa chọn --</option>
                                        <?php foreach($listCoupon as $key => $value){?>
                                            <option  <?= isset($old_field['coupon_id']) && $old_field['coupon_id'] == $value['id'] ? 'selected' : ''?> value="<?=$value['id']?>"><?=$value['name']?> --- <?=$value['code']?> </option>
                                        <?php }?>
                                </select>
                                <?= isset($errors['coupon_id']) ? '<span id="error-name" class="text-danger">'.$errors['coupon_id'].'</span>' : ''?>
                            </div>
                        </div>
                        <div class="col-md-4">
...
<script>
 var idDelete = false;
    function deleteAjax(dataPayload) {
        $.ajax({
                method: "POST",
                url: "/admin/product-coupon/delete",
                data: {
                    arr: dataPayload,
                }
            })
